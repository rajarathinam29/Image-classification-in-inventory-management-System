...
class SuperAdminUsersController extends Controller
{
    public function index() {
        $title='Users';
        return view('superadmin.users.users-list', compact('title'));
    }
    public function create() {
        $title='Create User';
        return view('superadmin.users.create', compact('title'));
    }
    public function edit() {
        $title='Edit User';
        return view('superadmin.users.edit', compact('title'));
    }
    public function view() {
        $title='View User';
        return view('superadmin.users.view', compact('title'));
    }
    ...
    public function getUser(Request $request) {
        $users = Users::with('role')->where('id', $request->userId)->first()->makeHidden(['password', 'token', 'permissions']);
        if(is_null($users)){
            return response()->json(['status' => FALSE, 'msg' => 'User not found.']);
        }
    ...
    public function updateUser(Request $request) {
        $validator = Validator::make($request->all(), [
            ...
            'last_name' => 'required',
            'email' => 'required',
            'user_name' => 'required|unique:users,user_name,'.$request->userId,
            'phone_no' => 'required',
            'street' => 'required',
            'city' => 'required',
            'state' => 'required',
    ...
    public function getUserLog(Request $request) {
        $users = DB::table('users_log')
                ->select('*')
                ->where('user_id', '=', $request->userId)
                ->orderBy('id', 'desc')
                ->get();
        if(!$users->isEmpty()){
            $this->addAdminLog($request->loginId, 'View Users log details.');
            $this->sendData(TRUE, $users, 'logData'); 
        }else{
            $this->addAdminLog($request->loginId, 'Trying to view user log details');
            $this->sendData(FALSE, 'User log not found');
        }
    }
}
