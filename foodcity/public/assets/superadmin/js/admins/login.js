        ...
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.status == true)
                {
                    ...
                    localStorage.setItem('DailyFoodCity', btoa(JSON.stringify(result.userData)));
                    setTimeout(function(){
                        if(result.userData.role_id){
                            location.assign(baseUrl+'/adminDashboard');
                        }else{
                            location.assign(baseUrl+'/selectBranch?id='+result.userData.id);
                        }
                    }, 1000);
                }
                else
