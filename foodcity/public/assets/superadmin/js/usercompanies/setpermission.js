var userCompanyData = [], modulesData = [];
var url_string = location.href; //window.location.href
var url = new URL(url_string);
var userCompanyIdData = JSON.parse(atob(url.searchParams.get("q")));
$(function(){
    getModules();
    // getUserCompany();
})
...
$('#btnSubmit').on('click', function(e){
    ...
    data.userCompanyId = userCompanyIdData.userCompanyId;

    $.ajax({
        url: `${baseUrl}/superAdmin/userCompanies/setPermission`,
        type: "POST",
        dataType: "json",
        data: data,
...
function getUserCompany(){
    ...
    data.userCompanyId = userCompanyIdData.userCompanyId;
    $.ajax({
        url: `${baseUrl}/superAdmin/userCompanies/getUserCompany`,
        type: "POST",
        dataType: "json",
        data: data,
        success: function(result){
            if(result.status == true)
            {
                userCompanyData = result.userCompanyData;
            }
            else
            {
                showError(result.msg);
...
function setUserPermission(){
    var permissions = JSON.parse(userCompanyData.permissions);
...
function getModules(){
    ...
    let data = adminData;
    $.ajax({
        url: `${baseUrl}/superAdmin/modules/getModules`,
        type: "POST",
        dataType: "json",
        data: data,
        success: function(result){
